...
function upMerge() {
  ...
  for (let i = 0; i < cells.length - fieldSize; i++) {
    if (isCellFilled(cells[i])) {
      const sumCard = cells[i].children[0];
      const current = sumCard.textContent;
...
function downShift() {
  ...
  for (let i = cells.length - fieldSize - 1; i >= 0; i--) {
    if (isCellFilled(cells[i])) {
      let newIndex;
...
function upShift() {
  ...
  for (let i = fieldSize; i < cells.length; i++) {
    if (isCellFilled(cells[i])) {
      let newIndex;
...
function checkForLose() {
  if ([...cells].some(cell => !isCellFilled(cell))) {
    return;
  }
